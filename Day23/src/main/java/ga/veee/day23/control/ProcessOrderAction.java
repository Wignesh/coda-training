...
public class ProcessOrderAction extends Action {
    ...
    public ProcessOrderAction(Properties dbConfigProp) {
        ProcessOrderAction.dbConfigProp = dbConfigProp;
    }
    ...
    public String execute(HttpServletRequest request, HttpServletResponse response) {
        try {
            ...
            if (!ids.isEmpty()) {
                ...
                BillingAddressDTO billingAddress = billingAddressDAOImpl.findById(addressId);
                ...
                executor.shutdown();
                if (insertInvoiceMaster > 0) {
                    attributesKV.forEach((k, v) -> {
                        request.getSession().removeAttribute(String.valueOf(k));
                    });
    ...
    private String getUsername(HttpServletRequest request) {
        return request.getSession().getAttribute("uname").toString();
    }
